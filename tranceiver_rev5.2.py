...
#    	self.abort = 0
    def DaemonThread(self):
        name=input("Need a name for the server :").strip()
    	...
    	print("serverDaemon=%s" % nameserverDaemon)
    	print("BC-server =%s" % broadcastServer)
    	...
    	# (a set provides fast lookup compared to a list)
    		...
    		rs=[nameserverDaemon]  # only the broadcast server is directly usable as a select() object
